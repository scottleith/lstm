...
class LSTM( object ):
	...
	def _predict( self, X ):
		...
		self._create_activity_caches( X.shape[0] )
		output = self._run_forward_pass( X )
		return output
	...
	def _run_epoch( self, epoch ):
		...
		batches_x, batches_y = gen_batches( shuffx, shuffy, self.mini_batch_size )

		for x_batch, y_batch in zip( batches_x, batches_y ):

			output = self._run_forward_pass( x_batch )
			self._run_backward_pass( y_batch, output )

		# Update error lists.
		self.train_err.append( _calculate_cost( self.loss_func, self._run_forward_pass( self.Xtrain ), self.Ytrain ) )
		self.val_err.append( _calculate_cost( self.loss_func, self._run_forward_pass( self.Xval ), self.Yval) )


	def _create_activity_caches( self, curr_batch_size ):
	...
	def _run_forward_cell_step( self, x, t ):
	...
	def _run_backward_pass( self, y_batch, output ):
		...
		self._initialize_gradient_caches()
		...
		for t in reversed( range( 0, self.n_timesteps ) ):
			...
			dh_next, dc_next = self._run_backward_cell_step( t, dh_next, dc_next )

		self._apply_gradients()

	def _initialize_gradient_caches( self ):
	...
	def _run_backward_cell_step( self, t, dh_next, dc_next ):
