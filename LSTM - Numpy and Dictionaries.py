...
train_set_X = np.array( sequences_X[0:9000], dtype = np.float32)
train_set_Y = np.array( sequences_Y[0:9000], dtype = np.float32)
val_set_X = np.array( sequences_X[9000:], dtype = np.float32)
val_set_Y = np.array( sequences_Y[9000:], dtype = np.float32)
...
def _initialize_weights( shape, initializer = 'glorot' ):
	...
	if initializer == 'glorot':
		...
		return np.random.normal( loc = 0, scale = np.sqrt( 1 /  shape[1] ), size = shape )
	elif initializer == 'he':
		return np.random.normal( loc = 0, scale = np.sqrt( 2 / shape[1] ), size = shape )
	else:
		return np.random.randn( shape[0], shape[1] ) * .01

def gen_batches( data_x, data_y, batch_size ):

	'''Takes in X and Y, splits it into batches of batch_size.
	Final batch is whatever remains (ie likely won't be of batch_size)'''
...
def _shuffle(X,Y):
	...
	''' Takes in X and Y, shuffles them together, returns X and Y. '''
